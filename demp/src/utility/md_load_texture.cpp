...
GLuint mdLoadTexture(void* data, mdEngine::u32 size)
{
	...
	GLint width, height, nrComponents;
	unsigned char* dataUC = stbi_load_from_memory((unsigned char*)data, size, &width, &height, &nrComponents, 0);
	...
	if (dataUC)
	{
		...
		glGenTextures(1, &textureID);
		glBindTexture(GL_TEXTURE_2D, textureID);
		glTexImage2D(GL_TEXTURE_2D, 0, format, width, height, 0, format, GL_UNSIGNED_BYTE, dataUC); // generate a texture
		glGenerateMipmap(GL_TEXTURE_2D); // Automatically generate all required mipmaps for currently bound texture
